...
def parse_args():
    ...
    parser.add_argument("--csv_file", type=str, default=None, help="path of csv file")
    parser.add_argument(
...
def main():
    ...
    clients = init_clients(num_clients=3)
    clients = split_dataset(path=args.csv_file, clients=clients)
    ...
    for client in clients:
        ...
        client.encode(save_dir=args.save_dir)

    concat_latent(clients)
    ...
    if args.train:
        ...
        if dataset.auto_norm:
            ...
            np.save(
                os.path.join(args.save_dir, f"ddpm_{args.mode}_{args.name}.npy"),
                samples,
            )
    else:
        trainer.load(args.milestone)
        ...
        samples = trainer.sample(
            num=len(dataset), size_every=2001, shape=[dataset.window, dataset.var_num]
        )
        if dataset.auto_norm:
            samples = unnormalize_to_zero_to_one(samples)
