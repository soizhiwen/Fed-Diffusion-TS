...
class Autoencoder(nn.Module):
    ...
    def train_with_settings(self, epochs, batch_sz, real_data, optimizer, loss_fn):
        ...
        for epoch in range(epochs):
            ...
            print(
                f"Epoch: {epoch} Loss: {train_loss[-1]:.3f}")
...
def train_vae_alone(EPOCHS = 200, BATCH_SIZE = 64, latent_dim = 3):
    ...
    outs, train_loss = model.train_with_settings(EPOCHS, BATCH_SIZE, data_tensor, optimizer, loss_fn)
    visualize(train_loss)

    np_outs = outs.detach().numpy()
    original_data = sc.inverse_transform(np_outs)
    pd.DataFrame(original_data, columns=df.columns).to_csv("Data/stock_data_synthetic.csv", index=False)
...
if __name__ == "__main__":
    ...
    mean_differences = calculate_mean_differences(BATCH_SIZE, original_csv = 'Data/stock_data.csv', synthetic_csv = 'Data/stock_data_synthetic.csv')
...
class FedAvgServer(DecentralizedServer):
    ...
    def forward(self, x):
        x, mu, logvar = [client.forward(self, x, forward_step = 'encode') for client in self.clients]
        x = np.concatenate(x)
        x = ddpm(x)
